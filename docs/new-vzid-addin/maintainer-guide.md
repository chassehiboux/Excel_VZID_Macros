...
- автообновление у пользователей идет не прямой заменой `xlam`, а через `updater.exe -> setup.exe`.
...
- не вставляй `separator` внутрь `box`: по RibbonX это ломает XML вкладки;
- если нужен визуальный отступ, сначала проверяй, что используемая комбинация контейнеров допустима для RibbonX.
...
### Что теперь реально обновляется у пользователя

- пользовательская надстройка больше не должна сама скачивать себе новый `MainVZID.xlam` для обычного обновления;
- штатный путь такой:
  `MainVZID` находит новую версию -> ставит обновление в очередь -> после закрытия Excel локальный `updater.exe` скачивает `setup.exe` -> запускает `setup.exe --no-ui`;
- файл `MainVZID.xlam` в GitHub Release остается в составе релиза для ручной проверки, диагностики и резервной установки из файла;
- если ты меняешь контракт `updater.exe`, поднимай `min-updater-version`, чтобы старая установленная версия пошла через переходный сценарий с локально скачанным `setup.exe`.

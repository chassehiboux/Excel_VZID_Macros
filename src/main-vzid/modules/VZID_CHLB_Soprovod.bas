Attribute VB_Name = "VZID_CHLB_Soprovod"
Option Explicit
...
' === Точка входа (пакетно) ===============================================
Sub CHLB_Make_Cover_Letters()
    Dim ws As Worksheet, wsRes As Worksheet
    Dim lastRow As Long, r As Long, blockRows As Long
    Dim templPath As String, vzyskName As String, tplResolved As String
    Dim baseFolder As String, outRoot As String
    Dim dictPerRecipient As Object
    Dim dictFilesPerRecipient As Object ' ключ -> Collection путей файлов по получателю
    ...
    ' --- Выбор взыскателя/шаблона (ЧЭС)
    Dim okSel As Boolean
    okSel = CHLB_SelectVzyskAndTemplate(False, vzyskName, templPath)
    ...
    ' --- Обновляем шаблон в %AppData%\Microsoft\Templates и используем локальную копию
    tplResolved = CHLB_ResolveTemplateToUserTemplates(templPath)
    ...
    totalBlocksAll = CountBlocks(ws, False)
    ...
    outRoot = CHLB_UniqueSubfolderPath(baseFolder, SanitizeFileName(d & " " & vzyskName))
    ...
    Do While r <= lastRow
        ...
        If okToMake Then
            Dim idx As Long, coll As Collection
            ...
            ' регистрируем путь для будущего "Общего"
...
NextR:
    ...
    For Each k In dictFilesPerRecipient.keys
        ...
        If filesColl.count > 1 Then
            ...
            combinedPath = AppendPath(AppendPath(outRoot, SanitizeFileName(CStr(k))), _
                                      SanitizeFileName(vzyskName & " - " & CStr(k) & " - Общий.docx"))
            CHLB_CreateCombinedDocFromList wdApp, filesColl, combinedPath
        End If
    Next k
...
' === ВХОД №2: по одному документу на получателя (WSS) =====================
Sub CHLB_Make_Cover_Letters_ByRecipient()
    Dim ws As Worksheet
    Dim lastRow As Long, r As Long, blockRows As Long
...
NextR:
    ...
    ' Выбор WSS-шаблона
    Dim okSel As Boolean
    okSel = CHLB_SelectVzyskAndTemplate(True, vzyskName, templPath)
    If Not okSel Then Exit Sub
    If Not SafeFileExists(templPath) Then MsgBox "Не найден шаблон Word:" & vbCrLf & templPath, vbCritical: Exit Sub

    ' Обновляем/используем локальную копию шаблона
    tplResolved = CHLB_ResolveTemplateToUserTemplates(templPath)
    ...
    If Len(baseFolder) = 0 Then Exit Sub
    today = Date: y = CStr(Year(today)): d = Format$(today, "dd.mm.yyyy")
    outRoot = CHLB_UniqueSubfolderPath(baseFolder, SanitizeFileName(d & " " & vzyskName & " для загрузки в WSS"))
    EnsureFolder outRoot
...
' === Выбор взыскателя/шаблона (теперь только Челябэнергосбыт) ============
Private Function CHLB_SelectVzyskAndTemplate(ByVal isWSS As Boolean, _
    ...
    vzyskName = "ЧЭС"
    If isWSS Then
        templPath = root & "Сопровод ЧЭС для WSS.docx"
    Else
        templPath = root & "Сопровод ЧЭС.docx"
    End If
    CHLB_SelectVzyskAndTemplate = True
End Function

' Возвращает путь к подпапке folderName внутри baseFolder. Если есть — добавляет " - n"
Private Function CHLB_UniqueSubfolderPath(ByVal baseFolder As String, ByVal folderName As String) As String
    ...
    If Not fso.FolderExists(p) Then CHLB_UniqueSubfolderPath = p: Exit Function
    n = 1
    Do
...
Private Function CHLB_ResolveTemplateToUserTemplates(ByVal netPath As String) As String
    ...
    If Len(Trim$(netPath)) = 0 Or Not fso.FileExists(netPath) Then
        CHLB_ResolveTemplateToUserTemplates = netPath
        Exit Function
    End If

    Dim localName As String: localName = fso.GetFileName(netPath)
    Dim localPath As String: localPath = templatesFolder & localName
    fso.CopyFile netPath, localPath, True  ' перезапись каждый запуск

    CHLB_ResolveTemplateToUserTemplates = localPath
    Exit Function
ErrHandler:
    On Error Resume Next
    CHLB_ResolveTemplateToUserTemplates = netPath
...
' === Создать общий документ из списка файлов (сохраняя формат/колонтитулы) ===
Private Sub CHLB_CreateCombinedDocFromList(ByVal wdApp As Object, ByVal filesColl As Collection, ByVal destPath As String)
    On Error GoTo ErrHandler
    If filesColl.count <= 1 Then Exit Sub ' только если более одного файла

    Dim baseDoc As Object, srcDoc As Object, lastSection As Object
    Dim i As Long, hfType As Variant
